...
const TONE: Record<SuggestionTone, { icon: typeof Info; className: string }> = {
  critical: { icon: AlertTriangle, className: "bg-destructive/10 text-destructive border-destructive/30" },
  ...
  positive: { icon: Sparkles, className: "bg-emerald-500/10 text-emerald-500 border-emerald-500/30" },
};
  ...
  return (
    <Card className="rounded-2xl border-border/60 bg-card/70 shadow-soft backdrop-blur">
      <CardHeader className="pb-3">
        ...
        <CardDescription>Recomendações geradas a partir das suas vendas, custos e estoque.</CardDescription>
      </CardHeader>
          ...
          suggestions.map((suggestion) => {
            ...
            const content = (
              <div className="flex items-start gap-3 rounded-xl border border-border/60 bg-background/40 p-3 transition-colors hover:bg-muted/40">
                <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg border ${tone.className}`}>
                ...
                {suggestion.to ? <ArrowRight className="mt-1 h-4 w-4 shrink-0 text-muted-foreground" /> : null}
              </div>
            );
