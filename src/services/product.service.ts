...
export const productService = {
  ...
  async create(input: ProductInput, companyId: string | null) {
    const { error } = await supabase.from("products").insert({ ...input, company_id: companyId });
    if (error) throw error;
  },
...
export const categoryService = {
  ...
  async create(input: CategoryInput, companyId: string | null) {
    const { error } = await supabase.from("categories").insert({ ...input, company_id: companyId });
    if (error) throw error;
  },
